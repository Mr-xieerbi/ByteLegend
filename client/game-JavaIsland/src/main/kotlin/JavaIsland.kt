...
const val STAR_BYTELEGEND_MISSION_ID = "star-bytelegend-challenge"
    ...
    npc {
        ...
        onInit = {
            when {
                ...
                playerChallenges.challengeAccomplished(STAR_BYTELEGEND_MISSION_ID) -> {
        ...
        onClick = helpers.standardNpcSpeech(guardId) {
            if (helpers.getCharacter(guardId).gridCoordinate == guardStartPoint) {
                when {
                    !gameRuntime.heroPlayer.states.containsKey(BEGINNER_GUIDE_FINISHED_STATE) && playerChallenges.challengeAccomplished(STAR_BYTELEGEND_MISSION_ID) -> {
